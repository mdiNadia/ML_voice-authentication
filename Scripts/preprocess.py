...
# حذف سکوت از صوت
def remove_silence(audio_data, sr, threshold=0.01, frame_length=2048, hop_length=512):
...
def label_to_number(label):
    if "_male.mp3" in label.lower():
        return 1
    elif "_female.mp3" in label.lower():
        return 0
...
def process_directory(input_directory, output_directory, output_csv, lowcut=50.0, highcut=5000.0, sr=22050, segment_duration=3):
    ...
    if output_data:
        ...
        print(f"Saved segments metadata to {output_csv}")
    else:
